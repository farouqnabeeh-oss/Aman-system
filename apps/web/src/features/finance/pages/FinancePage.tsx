...
function Transactions() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: () => api.get<any>('/finance/transactions', { params: { page } }).then((r) => r.data.data),
  });
  ...
  const saveMutation = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey:['transactions'] }); setEditOpen(false); toast.success('Operation Successful'); },
  });
      ...
      <Modal open={editOpen} onClose={() => setEditOpen(false)} title={editingId ? t.editTransaction : t.newTransaction}>
        <div className="space-y-8 pt-4">
           ...
           <div className="flex justify-end gap-4 mt-12 py-6 border-t border-white/5">
              ...
              <button className="clean-btn-primary px-10" onClick={() => saveMutation.mutate()} disabled={saveMutation.isPending}>{t.save}</button>
           </div>
        </div>
      </Modal>
    </div>
...
function Invoices() {
   ...
   const createMutation = useMutation({
      ...
      onSuccess: () => { toast.success('Created'); setCreateOpen(false); },
   });

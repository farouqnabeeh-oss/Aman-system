  ...
  return (
    <div className="space-y-12 pb-12">
      <PageHeader 
        ...
        action={<button onClick={() => {setEditingId(null); setForm({firstName:'', lastName:'', email:'', password:'', role:'EMPLOYEE', department:'', position:''}); setEditOpen(true);}} className="clean-btn-primary h-12 gap-2 text-[10px] uppercase tracking-widest"><UserPlus size={16} /> {t.addUser}</button>}
      />
      ...
      {isLoading ? (
        ...
        <div className="clean-card !p-0 overflow-hidden">
           <Table columns={[
              ...
              { key: 'actions', label: '', render: (u: any) => <div className="flex justify-end gap-2"><button onClick={() => handleEdit(u)} className="p-2 rounded-xl hover:bg-white/10 transition-all"><Edit2 size={14}/></button></div> }
           ]} data={users} keyFn={u => u.id} />
